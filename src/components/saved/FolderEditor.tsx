 ...
 * leaves it open with what was typed, for another go.
 */
export function FolderEditor({
  ...
  return (
    <BottomSheet open={open} onClose={onClose} title={title}>
      ...
      {open && <EditorForm initial={initial} preview={preview} submitLabel={submitLabel} onSubmit={onSubmit} onDone={onClose} />}
    </BottomSheet>
  );
}
  ...
  return (
      ...
      >
        <div className="flex items-center gap-4">
          <FolderArt
            folder={{ id: preview?.id ?? "new", emoji: draft.emoji, color: draft.color, coverUrl: preview?.coverUrl ?? null, covers: preview?.covers ?? [] }}
            variant="tile"
            ...
            {FOLDER_COLORS.map((c) => {
              ...
              return (
                <button
                  ...
                  aria-label={c.label}
                  aria-pressed={on}
                  className={`h-9 w-9 rounded-full transition-transform ${on ? "scale-110 ring-2 ring-foreground ring-offset-2 ring-offset-elevated" : "active:scale-95"}`}
