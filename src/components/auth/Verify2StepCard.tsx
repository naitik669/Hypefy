...
/**
 * Code entry for both email flows: the second factor after a password, and
 * passwordless sign-in where the code IS the credential. Past this point
 * the two are the same exchange, so they share a screen and differ only in
 * what the heading claims happened.
 */
export function Verify2StepCard({
  email,
  reason = "two-step",
}: {
  email: string;
  ...
  if (!email) {
    return (
      <div className="w-full max-w-[360px] text-center">
        ...
        <Link
          href="/signin"
          className="mt-2 inline-block font-semibold text-foreground hover:text-accent"
        >
          Back to sign in
        </Link>
      </div>
      ...
      <div className="animate-rise relative overflow-hidden rounded-[28px] border border-white/10 bg-white/[0.04] p-7 shadow-[0_30px_80px_-20px_rgba(0,0,0,0.8)] backdrop-blur-2xl">
        ...
        <div className="flex flex-col items-center text-center">
          ...
          <h1 className="mt-3 text-2xl font-bold tracking-tight text-foreground">
            {reason === "otp" ? "Check your email" : "Two-step verification"}
          </h1>
        ...
        <p className="mt-6 text-center text-xs text-muted">
          ...
          <button
            ...
            onClick={resend}
            disabled={cooldown > 0}
            className="font-semibold text-foreground transition-colors hover:text-accent disabled:opacity-60"
          >
